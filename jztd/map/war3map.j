...
function EverySecond_Conditions takes nothing returns boolean
    ...
    if passed_time == 10 then
        ...
        call DisplayTimedTextToForce(bj_FORCE_ALL_PLAYERS, 10, "|cff00ff00[小贴士]|r|cfffff000请在NPC天下门派处选择门派开始游戏，收藏游戏的玩家可以在地图中间的商店中领取福利礼包！|r")
    endif
    if passed_time == 20 then
        call DisplayTimedTextToForce(bj_FORCE_ALL_PLAYERS, 15, "|cff00ff00[小贴士]|r|cfffff000游戏前期金钱较匮乏，可以合理使用木材兑换金钱，关于游戏的各个系统，请查看F9任务面板，欢迎在地图留言区提出您的宝贵意见|R")
    endif
    if passed_time == 40 then
        call DisplayTimedTextToForce(bj_FORCE_ALL_PLAYERS, 15, "|cff00ff00[小贴士]|r|cfffff000前期防御塔建立在合理的位置，可以提高输出效率。高级防御塔性价比通常高于低级防御塔，因此建议将资源优先用于升级防御塔|R")
    endif
    if passed_time == 60 then
        call DisplayTimedTextToForce(bj_FORCE_ALL_PLAYERS, 15, "|cff00ff00[小贴士]|r|cfffff000游戏里的相关指令请按F9打开任务面板进行查询|r")
    endif
    if passed_time == 80 then
        call DisplayTimedTextToForce(bj_FORCE_ALL_PLAYERS, 15, "|cff00ff00[小贴士]|r|cfffff000由于制作和测试时间紧张，如遇游戏里有BUG或不舒适的体验给你带来不愉快，尽情谅解。|R")
    endif
    if passed_time == 100 then
        call DisplayTimedTextToForce(bj_FORCE_ALL_PLAYERS, 15, "|cff00ff00[小贴士]|r|cfffff000塔升级到顶级后，如果有技能还没修炼到满级，可以使用武学修炼丹继续修炼。|R")
    endif
    // if passed_time == 120 then
    //     call DisplayTimedTextToForce(bj_FORCE_ALL_PLAYERS, 15, "|cff00ff00[小贴士]|r|cfffff000生存模式前期强度较高，尽量在普通模式下积累一些积分后再进行尝试。|R")
    // endif
    // if passed_time == 140 then
    //     call DisplayTimedTextToForce(bj_FORCE_ALL_PLAYERS, 15, "|cff00ff00[小贴士]|r|cfffff000生存模式下从第31波（第一次获得）起每10波获得5点存档积分。|R")
    // endif
    // if passed_time == 160 then
    //     call DisplayTimedTextToForce(bj_FORCE_ALL_PLAYERS, 15, "|cff00ff00[小贴士]|r|cfffff000生存模式第60波后将不再给予每波金钱和珍稀币奖励。|R")
    // endif
...
call ExecuteFunc("jasshelper__initstructs1065578")
...
function jasshelper__initstructs1065578 takes nothing returns nothing
