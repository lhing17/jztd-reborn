...
function dealDamage takes unit u,unit ut,real damage returns nothing
    ...
    if GetUnitAbilityLevel(u, 'A03N') >= 1 then
        set coeff=coeff + 0.1 + 0.1 * GetUnitAbilityLevel(u, 'A03N')
    endif
    // 太玄神功 伤害增加50%
    if GetUnitAbilityLevel(u, 'A03P') >= 1 then
        set coeff=coeff + 0.2 + 0.1 * GetUnitAbilityLevel(u, 'A03P')
    endif
    // 蛤蟆功 伤害增加40%
    if GetUnitAbilityLevel(u, 'A03Q') >= 1 then
        set coeff=coeff + 0.1 + 0.1 * GetUnitAbilityLevel(u, 'A03Q')
    endif
    // 洗髓经 伤害增加50%
    if GetUnitAbilityLevel(u, 'A03R') >= 1 then
        set coeff=coeff + 0.2 + 0.1 * GetUnitAbilityLevel(u, 'A03R')
    endif
    // 塔的伤害加成
...
function showTowerTooltip takes nothing returns nothing
 ...
 local real damageAddition= 0
	if DzGetTriggerUIEventPlayer() == GetLocalPlayer() then
		...
		call DzFrameSetText(s__Frame_id[(towerTooltipWidget[4])], ("武学暴击率：" + I2S(5 + LoadInteger(TOWER_ATTR_HT, GetHandleId(unitInSelection[i]), TOWER_CRITICAL_RATE_KEY)) + "%")) // INLINED!!
		call DzFrameSetText(s__Frame_id[(towerTooltipWidget[5])], ("暴击倍数：" + R2S(2 + LoadReal(TOWER_ATTR_HT, GetHandleId(unitInSelection[i]), TOWER_CRITICAL_ADDITION_KEY)))) // INLINED!!
...
call ExecuteFunc("jasshelper__initstructs667982500")
...
function jasshelper__initstructs667982500 takes nothing returns nothing
